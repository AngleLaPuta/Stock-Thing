...
def getyear(provider, instrument, year, price_calculation='close', time_group=None, drop_non_price_columns=False):
    '''
...
def get_date(start_date, end_date, instrument, provider='oanda', price_calculation='close', time_group=None, drop_non_price_columns=False):
    '''
    This will get data within a specific date range (start_date to end_date).

    It calculates the years required from the date range, loads the data for those
    years, and then filters the resulting dataframe by the exact date range.
    The dates should be provided as strings (e.g., 'YYYY-MM-DD').
    '''
    # Convert dates to datetime objects to extract years
    start = pd.to_datetime(start_date)
    end = pd.to_datetime(end_date)

    # Get the range of years needed to load the raw data
    start_year = start.year
    end_year = end.year
    years = [str(y) for y in range(start_year, end_year + 1)]

    # Load data for all required years using the existing multi-year function
    price_data = get_multi_year(
        years=years,
        instrument=instrument,
        provider=provider,
        price_calculation=price_calculation,
