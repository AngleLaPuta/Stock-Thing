...
def plot_comparison_results(analysis_df: pd.DataFrame, all_strategies: list, ticker_symbol: str, plot: bool):
    ...
    filtered_strategies_for_ax4 = [
        s for s in all_strategies
        if 'agent' in s.strategy_name.lower() or 'perfect' in s.strategy_name.lower()
    ]
    for i, strategy in enumerate(all_strategies):
        color = color_pool(i)
        strategy_colors[strategy.strategy_name] = color
        if strategy not in filtered_strategies_for_ax4:
            continue
        ...
        sell_dates_signal = filter_trades_by_date(strategy.sell_dates_signal + strategy.sell_dates_eod, display_date)
        sl_dates = filter_trades_by_date(strategy.sell_dates_stoploss, display_date)
        first_signal_drawn = False
        if buy_dates:
            ax1.axvline(x=buy_dates[0], ymin=0, ymax=1, color=color, linestyle='--', linewidth=1, alpha=0.8, zorder=5,
                        label=f'{strategy_short_name} Buy/Sell{label_suffix}')
            ax4.axvline(x=buy_dates[0], ymin=0, ymax=1, color=color, linestyle='--', linewidth=1, alpha=0.3,
                        zorder=0)
            first_signal_drawn = True
            for date in buy_dates[1:]:
                ax1.axvline(x=date, ymin=0, ymax=1, color=color, linestyle='--', linewidth=1, alpha=0.8, zorder=5)
                ax4.axvline(x=date, ymin=0, ymax=1, color=color, linestyle='--', linewidth=1, alpha=0.3, zorder=0)
        if sell_dates_signal:
            if not first_signal_drawn:
                ax1.axvline(x=sell_dates_signal[0], ymin=0, ymax=1, color=color, linestyle='--', linewidth=1, alpha=0.8,
                            zorder=5,
                            label=f'{strategy_short_name} Buy/Sell{label_suffix}')
                ax4.axvline(x=sell_dates_signal[0], ymin=0, ymax=1, color=color, linestyle='--', linewidth=1, alpha=0.3,
                            zorder=0)
            for date in sell_dates_signal:
                ax1.axvline(x=date, ymin=0, ymax=1, color=color, linestyle='--', linewidth=1, alpha=0.8, zorder=5)
                ax4.axvline(x=date, ymin=0, ymax=1, color=color, linestyle='--', linewidth=1, alpha=0.3, zorder=0)
        if sl_dates:
            ax1.axvline(x=sl_dates[0], ymin=0, ymax=1, color='red', linestyle='-', linewidth=2, alpha=0.7, zorder=5,
                        label=f'{strategy_short_name} Stop-Loss')
            ax4.axvline(x=sl_dates[0], ymin=0, ymax=1, color='red', linestyle='-', linewidth=2, alpha=0.5,
                        zorder=0)
            for date in sl_dates[1:]:
                ax1.axvline(x=date, ymin=0, ymax=1, color='red', linestyle='-', linewidth=2, alpha=0.7, zorder=5)
                ax4.axvline(x=date, ymin=0, ymax=1, color='red', linestyle='-', linewidth=2, alpha=0.5, zorder=0)
    ax1.legend(loc='upper left', bbox_to_anchor=(1.05, 1.0), fontsize=7, ncol=1, title="Trade Signals")
    ax1.set_title(f'{ticker_symbol} Price Action & Trade Signals on {display_date} (Filtered Strategies)',
    ...
    for strategy in filtered_strategies_for_ax4:
    ...
    ax4.set_title(f'Portfolio Value Comparison Over Time (One Day) - {ticker_symbol} (Agent & Perfect Strategies Only)',
...
def amazingprogram(ticker, plot=True):
    ...
    try:
        ...
        strategies = [
            ...
            PPOStrategy(model_ticker=MODEL_TICKER, initial_capital=initial_capital,
                        stop_loss_percent=stop_loss, model_dir='ppo_results'),
            ...
            GRUStrategy(model_ticker=MODEL_TICKER, initial_capital=initial_capital,
                        stop_loss_percent=stop_loss, model_dir='gru_results'),
            XGBoostStrategy(model_ticker=MODEL_TICKER, initial_capital=initial_capital,
                            stop_loss_percent=stop_loss, model_dir='supervised_results'),
            RandomForestStrategy(model_ticker=MODEL_TICKER, initial_capital=initial_capital,
                                 stop_loss_percent=stop_loss, model_dir='supervised_results'),
